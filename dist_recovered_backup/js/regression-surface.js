...
class RegressionSurfaceLab {
    ...
    renderCoordinateFloor() {
        ...
        for (let i = 0; i <= steps; i++) {
            ...
            this.ctx.strokeStyle = `rgba(15, 23, 42, ${fogAlpha.toFixed(3)})`;
            this.ctx.beginPath();
    ...
    renderFittedSurface(now) {
        ...
        for (const q of quads) {
            ...
            this.ctx.strokeStyle = `rgba(${r + 40}, ${g + 40}, ${b + 40}, ${(depthAlpha * 0.85).toFixed(3)})`;
    ...
    updateHUD() {
        ...
        if (this.hudSig) {
            ...
            const fStat = (activeMetrics.r2 / pParams) / (Math.max(1e-5, 1 - activeMetrics.r2) / dfRes);

            if (fStat > 12 || activeMetrics.r2 > 0.75) {
                this.hudSig.textContent = 'p < 0.001';
                this.hudSig.style.color = '#059669';
            } else if (fStat > 4 || activeMetrics.r2 > 0.45) {
                this.hudSig.textContent = 'p = 0.006';
                this.hudSig.style.color = '#10b981';
            } else if (fStat > 2 || activeMetrics.r2 > 0.25) {
                this.hudSig.textContent = 'p = 0.042';
                this.hudSig.style.color = '#d97706';
            } else {
                this.hudSig.textContent = 'p = 0.18 (n.s.)';
        ...
        if (this.hudFormula) {
            if (this.activeModel === 'ols') {
                ...
                const signB2 = b2 >= 0 ? '+' : '-';
                const signB0 = b0 >= 0 ? '+' : '-';
                this.hudFormula.textContent = `ŷ = ${b1.toFixed(2)}x₁ ${signB2} ${Math.abs(b2).toFixed(2)}x₂ ${signB0} ${Math.abs(b0).toFixed(2)}`;
                ...
                const signB4 = b4 >= 0 ? '+' : '-';
                const signB5 = (b5 || 0) >= 0 ? '+' : '-';
                this.hudFormula.textContent = `ŷ = ${b1.toFixed(2)}x₁ ${signB2} ${Math.abs(b2).toFixed(2)}x₂ ${signB3} ${Math.abs(b3).toFixed(2)}x₁² ${signB4} ${Math.abs(b4).toFixed(2)}x₂² ${signB5} ${Math.abs(b5 || 0).toFixed(2)}x₁x₂`;
